...
Accounts.prototype.privateKeyToAccount = function privateKeyToAccount(privateKey, targetAddressRaw) {
  ...
  let account = Account.fromPrivate(privateKey)
  
  
  if (targetAddressRaw) {
    const isHumanReadable = utils.isHumanReadableString(targetAddressRaw)
    
    account.address = isHumanReadable
      ? utils.humanReadableStringToHexAddress(targetAddressRaw)
      : targetAddressRaw
  }
...
Accounts.prototype.encrypt = function (privateKey, password, options) {
    ...
    let address = options.address || account.address
    let addressAsHumanReadableString
    
    const isHumanReadable = utils.isHumanReadableString(address)
    if (isHumanReadable) {
      addressAsHumanReadableString = address
      address = utils.humanReadableStringToHexAddress(address)
    }
    ...
    return {
        ...
        addressAsHumanReadableString: addressAsHumanReadableString,
        crypto: {
...
Wallet.prototype.add = function (account, targetAddressRaw) {
    ...
    if (targetAddressRaw) this[targetAddressRaw] = account // this['toshi']
    this.length++
